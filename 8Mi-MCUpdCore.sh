...
curseforge_getLatestDLURL(){
    cf_json=`curl -s "https://api.cfwidget.com/"$1"?version="$2`
...
modrinth_getLatestDLURL(){
    if [ !$2 ]; then
    modrinth_gameversion=`curl -s https://api.modrinth.com/v2/tag/game_version | jq -r .[0].version`
    fi
    curl -s https://api.modrinth.com/v2/project/$1/version?game_versions=[%22${2:-$modrinth_gameversion}%22] | jq -r .[0].files[0].url >&1
}
